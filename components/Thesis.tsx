  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `radial-gradient(350px circle at ${cursor.x}px ${cursor.y}px, rgba(212,168,67,0.07), transparent 60%)`,
        }}
      />
      ...
      <div className="absolute top-6 right-6 text-right">
        <div className="text-xl font-display font-bold text-[#D4A843] opacity-50 group-hover:opacity-100 transition-opacity duration-300">
          {pillar.stat}
        </div>
        <div className="text-[10px] text-[#6B6560] tracking-widest uppercase mt-0.5">{pillar.statLabel}</div>
      </div>

      {/* Icon */}
      <div className="text-3xl mb-5 group-hover:scale-110 transition-transform duration-300 inline-block origin-left">
        {pillar.icon}
      </div>

      {/* Label */}
      <p className="text-[10px] font-bold tracking-[0.2em] text-[#D4A843]/60 uppercase mb-1.5">{pillar.subtitle}</p>

      {/* Title */}
      <h3 className="font-display text-lg font-bold text-[#F2EDE8] mb-3 leading-snug">{pillar.title}</h3>

      {/* Description */}
      <p className="text-sm text-[#A89F94] leading-relaxed mb-5">{pillar.description}</p>
      ...
      <div className="flex flex-wrap gap-2">
        {pillar.tags.map((tag) => (
          <span
            key={tag}
            className="px-2.5 py-1 text-[10px] font-semibold tracking-wider border border-[#D4A843]/14 text-[#D4A843]/50 rounded-full group-hover:border-[#D4A843]/28 group-hover:text-[#D4A843]/75 transition-all duration-300"
          >
            {tag}
          </span>
        ))}
...
export default function Thesis() {
  return (
    <section id="thesis" className="py-24 lg:py-32 relative overflow-hidden bg-[#111110]">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_70%_50%_at_50%_50%,rgba(212,168,67,0.03),transparent)]" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#D4A843]/20 bg-[#D4A843]/[0.06] mb-6">
            <span className="text-[10px] font-bold tracking-[0.25em] text-[#D4A843] uppercase">Investment Thesis</span>
          </div>
          ...
          <p className="text-[#A89F94] max-w-2xl mx-auto text-base lg:text-lg leading-relaxed">
        ...
        <motion.p
          ...
          whileInView="visible"
          viewport={{ once: true }}
          className="mt-10 text-center text-xs text-[#6B6560] max-w-2xl mx-auto leading-relaxed"
        >
